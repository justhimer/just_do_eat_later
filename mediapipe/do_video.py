...
cap = cv2.VideoCapture("mediapipe/20230226_173824 1.mp4")  # 0 is the number representing my webcam
...
with mp_pose.Pose(
  ...
  while cap.isOpened():
      ...
      try:
        ...
        # visualize angle
        cv2.putText(image,
                    f'{joint_status_description[left_arm_status]}',
                    tuple(np.multiply(left_elbow, [cap_width, cap_height]).astype(int)),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2, cv2.LINE_AA)
        cv2.putText(image,
                    f'{joint_status_description[right_arm_status]}',
